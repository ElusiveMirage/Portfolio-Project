...
public class IC_Tower : UnitBase
{
    ...
    private void IceField()
    {
        ...
        else
        {
            ...
            foreach (Collider2D potentialTarget in enemiesInRange)
            {
                potentialTarget.GetComponent<TD_EnemyBase>().RestoreMoveSpeed();
            }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (skillActive)
        {
            ...
            {
                collision.gameObject.GetComponent<TD_EnemyBase>().ModifyMoveSpeed(0.6f);
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "TD_Enemy")
        {
            collision.gameObject.GetComponent<TD_EnemyBase>().RestoreMoveSpeed();
        }
    }
